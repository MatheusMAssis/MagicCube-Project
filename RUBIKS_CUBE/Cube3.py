...
class Cube3:
    ...
    def __init__(self, state=[0]*9 + ([1]*3 + [2]*3 + [3]*3 + [4] * 3)*3 + [5]*9):
        self.state = state
    ...
    #--- scrambling cube with n random movements ---#
    def scramble(self, n):
        scramble_list = []
        movement_list = ['U', 'Ui', 'D', 'Di', 'L', 'Li', 'R', 'Ri', 'F', 
                         'Fi', 'B', 'Bi']
        choice = int(random(len(movement_list)))
        movement = movement_list[choice]
        scramble_list.append(movement)
        self.move(movement)
        while len(scramble_list) < n:
            next_choice = int(random(len(movement_list)))
            next_movement = movement_list[next_choice]
